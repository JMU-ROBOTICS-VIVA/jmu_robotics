...
def generate_launch_description():
    ...
    map_cmd = DeclareLaunchArgument(
        'map',
        default_value=PathJoinSubstitution([pkg_turtlebot4_navigation,
        'maps',
        'warehouse.yaml'
        ]),
    ...
    # The shell intentionally exposes an absolute ROS namespace such as
    # /robotsim1.  The upstream TurtleBot/Create 3 Gazebo launch code also
    # reuses this value while constructing Gazebo model/topic names, where a
    # leading slash produces malformed paths such as /model//robotsim1/... .
    # Strip leading/trailing slashes internally. PushRosNamespace still
    # resolves the ROS graph under the expected absolute /robotsim1 namespace.
    namespace = PythonExpression([
        "'", LaunchConfiguration('namespace'), "'.strip('/')"
    ])
    use_sim_time = LaunchConfiguration('use_sim_time')

    world = LaunchConfiguration('world') # kpmadd
